...
$(function() {
    ...
    //функция динамического обновления списка тикетов
    function updateTicketPlan(ticket_id, action, text) {          
      
        ticket_id = parseInt(ticket_id);
        if (ticket_id > 0) {          
            var url = document.location.href;
            
            var row_id = "#ticket-row-" + ticket_id;
            //отправка запроса
            $.post(url, {AJAX: "YES", ACTION: action, TICKET_ID: ticket_id, COMMENT: text}, function(data){
                $(row_id).load(url + " " + row_id + " > *"); 
            })
            //перезагрузка нужной области
            $(row_id).load(url + " " + row_id + " > *");
        }
    }

    //управление всплывающими окнами
    $("body").on("click", ".js-ticket-plan-control", function() {
        $(".ticket_plan_popup").fadeOut();
        $(this).siblings(".ticket_plan_popup").fadeToggle();
    })   
    //закрытие всплывающего окна
    $("body").on("click", ".ticket-tracking-close", function() {            
        $(this).parent(".ticket_plan_popup").fadeToggle();
        $(".js-ticket-plan-comment").val("");
    })
    //закрытие при клике на свободной области
    $("body").on("click", function(e){     
        if(!$(e.target).parents().hasClass("ticket_plan_popup") && !$(e.target).hasClass("js-ticket-plan-control") && !$(e.target).hasClass("ticket_plan_popup")) {
            $(".ticket_plan_popup").fadeOut();
        }   
    })  

    //add to plan
    $("body").on("click", ".js-add-to-plan", function(){
        var ticket_id = $(this).data("ticket-id");  
        var action = "ADD";
        updateTicketPlan(ticket_id, action);

    }) 


    //проверка на ввод текста во всплывающих окнах
    $("body").on("keyup", ".js-ticket-plan-comment", function() {
        if ($(this).val() != "") {
    ...
    $("body").on("click", ".js-add-ticket-plan-comment", function(e) {
        e.preventDefault();
        var ticket_id = $(this).data("ticket-id");
        var action = $(this).data("action");
        var text =  $(this).siblings(".ticket-plan-update-text-" + ticket_id).val();        
        updateTicketPlan(ticket_id, action, text);
    }) 
