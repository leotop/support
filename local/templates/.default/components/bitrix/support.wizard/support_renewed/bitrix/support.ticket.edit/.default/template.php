...
<?if (in_array(8, $USER->GetUserGroupArray())){?> 
    ...
    <script type="text/javascript">
        ...
        <? if($_POST["NEW_TICKET"]==1) {?>
            ...
            <?} else {?>
            //Проверка на выбранный статус при заполненном сообщении
            function check_status() {  
                //alert($("#MESSAGE").val());           
                if(!($("#MESSAGE").val()=="")) {

                    if (!($("#STATUS_SID").val()=='D' || $("#STATUS_SID").val()=='G' || $("#STATUS_SID").val()=='L' || $("#STATUS_SID").val()=='B') ) {
                        if ($("#STATUS_SID").val() == 'W' && $("#saveStatus").prop("checked") == true) {     //для сохранения статуса "в работе" появляется отдельный чекбокс
                            check_time();  
                        }
                        else {
                            alert('Установите статус "Разрешен", "Требуется уточнение клиента", "Отложен", либо "Невалидный"');
                            return false;
                        }
                    }  else {  
                        check_time();
        ...
        function check_time() {
            if($("#STATUS_SID").val()=='G') {
                if($("#SPEND_TIME").val()=='') {
                    alert('Поле "Затраченное время" не должно быть пустым');
                    return false;
                } else { 
                    $("#ticket-form").submit();
                }
...
<br>
<br>
<h5>Тикет №<?=$arResult["TICKET"]["ID"]?>. <span style="text-transform: lowercase;"><?=$arResult["TICKET"]["TITLE"]?></span></h5>
        ...
        <table class="support-ticket-edit data-table">
            ...
            <tr>
                <td>
                    ...
                    <?if (in_array(8, $USER->GetUserGroupArray())){?>
                        <?  
                            //собираем пользователей из группы "сотрудники техподдержки"
                        ...
                        <?  
                            ...
                            <label><input type="checkbox" name="saveStatus" id="saveStatus" value="1" > сохранить статус "В работе"</label>
                            <?}?>
                           <br><br> 
                            <label><input type="checkbox" id="UF_IN_PAYMENT" name="UF_IN_PAYMENT" value="1" <?if ($arResult["TICKET"]["UF_IN_PAYMENT"]){?> checked="checked"<?}?>> УЧАСТВУЕТ В ОПЛАТЕ</label>
                    ...
                    <?if (in_array(8, $USER->GetUserGroupArray()) || in_array(7, $USER->GetUserGroupArray()) || $USER->IsAdmin()){?>
                        ...
                        <?if($_GET['ID']<=1235 && in_array(8, $USER->GetUserGroupArray())){?>
                            ...
                            <a href="javascript:void(0)" class="billingTableControl">
                                <span class="addHours">Управление биллингом</span>
                                <span class="hideBillingTable">Скрыть</span>
                            </a>
                            <br><br>
                                ...
                                <table>
                                    ...
                                    <?if(in_array(8, $USER->GetUserGroupArray()) || $USER->IsAdmin()){?>
                                        <tr class="billingEditRow">
                                            <td>Будет добавлена текущая дата.</td>
                                            <td>Время будет учтено от<br>имени текущего сотрудника.</td>
                                            <td class="tableHours">                                            
                                                <div>
                                            ...
                                            <td>
                                                ...
                                                <input class="submitBilling" data-action="add" value="Добавить" type="submit" />
                                            </td>
                                        </tr>
                                        <?}?>
                                </table>
            ...
            <tr>
                <td>
                    <?=$arResult["NAV_STRING"]?>
                    <?$userGroups = array();//массив с группами пользователей, которые пишут посты?>
                    <?foreach ($arResult["MESSAGES"] as $arMessage):?>
                        <div class="ticket-edit-message">
                            ...
                            <?
                                //если автор поста является адинистратором - выводим соответствующее пояснение в скобках
                            ...
                            ?>
                            <?if (in_array(1,$userGroups[$arMessage["OWNER_USER_ID"]])) {?><i class="ticket_link">(Администратор техподдержки)</i><?} else if (in_array(7,$userGroups[$arMessage["OWNER_USER_ID"]])) {?>(<?=$arUsers[$arMessage["OWNER_USER_ID"]]?>)<?}?>
                            <br />
