...
<?if (in_array(8, $USER->GetUserGroupArray())){?> 
    ...
    <script type="text/javascript">
        ...
        function check_status() {  
            //alert($("#MESSAGE").val());           
            if(!($("#MESSAGE").val()=="")) {
                //alert($("#STATUS_SID").val()); 
                if (!($("#STATUS_SID").val()=='D' || $("#STATUS_SID").val()=='G')) {
                    alert('Статус обращения должен быть "Разрешен" либо "Требуется уточнение клиента"');
                    return false;
                }  else {  
                    check_time();
                }
            ...
            <tr>
                <td>
                    ...
                    <?if ($USER->IsAdmin()){?>
                        <?
                            //собираем пользователей из группы "сотрудники техподдержки"
                            $user = CUser::GetList(($by="id"), ($order="asc"), array("GROUPS_ID"=>array(8)));   
                        ?>
