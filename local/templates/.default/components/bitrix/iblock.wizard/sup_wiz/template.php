            ...
            <tr>
                ...
                <td style="border:none" align=left>
                    <table cellspacing=0 cellpadding=8 border=0>
                            ...
                            if (count($arResult['FIELDS']))
                            {
                                ...
                                foreach($arResult['FIELDS'] as $num=>$f)
                                {
                                    ...
                                    if ($f['FIELD_TYPE']=='text') // simple input field
                                        ...
                                        '<input name="wizard['.$id.']" size=88 value="'.$f['FIELD_VALUE'].'">' .
                        ...
                        <?  //добавляем выпадающий список "создать от имени" 
                            $user_id = $USER->GetID();
                            in_array(1, CUser::GetUserGroup($user_id));
                            if ((in_array(1, CUser::GetUserGroup($user_id))) || (in_array(8, CUser::GetUserGroup($user_id)))) {
                            ?>
                            <tr>
                                <td>
                                    <?
                                        ...
                                        $users = CUser::GetList(($by="UF_SITE"), ($order="asc"), $filter, $params);
                                    ?>
                                    <div class="wizard_field_name"><?=GetMessage('AUTHOR')?>:</div>
                                    <select id="ticket_author" name="ticket_author">
                                        <option></option>
                                        <?  
                                            while ($arUser = $users->Fetch()) {
                                            ?>

                                            <option value="<?=$arUser["ID"]?>"><?=$arUser["UF_SITE"]?></option>
                                            <?
                                            }                                    
                                        ?>
                                    </select>
